...
- [x] 定义了携带状态码的错误类型（或用其他方式传递状态码）
- [x] `Chat`（和 `ChatStream`）的非 200 分支返回的是 `APIError` 而非 `fmt.Errorf`（接线步骤，漏了分类静默失效）
- [x] 区分了可重试（429/5xx/网络错误）与不可重试（其他 4xx）错误
- [x] 不可重试错误立即返回，不进入重试循环
- [x] 循环次数正确：1 次首发 + maxRetries 次重试（不是 range maxRetries）
- [x] 退避间隔指数增长（1s/2s/4s 或类似）
- [x] 有最大重试次数上限，最终失败时返回带上下文的错误
- [x] 最终错误用 `%w` 包装保留原始错误链
- [x] 验证方式可行：改错 baseURL 触发网络错误（走默认可重试分支）；验证 5xx 分类可指向本地 mock 服务；验证"401 不重试"需把 API key 改错（前提是上一条接线已完成）
