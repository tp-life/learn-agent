...
- [x] 请求体正确带了 `"stream": true`（且 `Stream` 字段有正确的 json tag）
- [x] 用独立的 `streamChunk` 结构解析 delta，而不是复用非流式的 `ChatResponse`
- [x] delta 外层 json tag 是 `delta`，不是 `content`
- [x] 正确处理了 `[DONE]` 结束标志（在 JSON 解析之前判断）
- [x] 跳过了非 `data:` 开头的行（空行、keep-alive 注释）
- [x] 考虑了 Scanner 单行长度上限问题
- [x] tool_calls 分片按 index 聚合，arguments 拼接完整（问一道计算题验证工具仍被触发）
- [x] 返回值包含完整 Message（content + tool_calls），assistant 消息能原样入历史
- [x] 流式接在 agent 循环内部，而不是绕过 `Run`（验证：多轮对话不失忆、工具调用正常）
- [x] 流式过程中连接中断有错误处理（`scanner.Err()`）
- [x] 运行效果：最终回答逐字/逐段打印出来，而非一次性出现
